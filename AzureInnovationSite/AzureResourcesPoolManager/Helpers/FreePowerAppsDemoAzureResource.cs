...
{
    public class FreePowerAppsDemoAzureResource : IFreeAzureResource
    {
        ...
        private readonly string resourceGroup = "AzureAppsAutoglass";
        AdOptions adOptions;
        AzureDemosDBContext dBContext;
        public FreePowerAppsDemoAzureResource(AdOptions adOptions, AzureDemosDBContext dBContext)
        {
        ...
        private async Task<string> DeleteLicenses(string userPrincipalName)
        {
            ...
            using (HttpClient httpClient = new HttpClient())
            {
                ...
                var newLicenses = new
                {
                    ...
                    removeLicenses = licenses.Where(l => l.SkuPartNumber == "POWERAPPS_PER_USER").Select(l => l.SkuId).ToArray()
                };
